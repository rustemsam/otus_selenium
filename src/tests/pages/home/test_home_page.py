...
def test_information_section(browser):
    ...
    expected_list = ['Terms & Conditions', 'Delivery Information', 'About Us', 'Privacy Policy']

    assert (expected_list == list_of_elements
...
def test_categories_section(browser):
    ...
    expected_list = ['Desktops', 'Laptops & Notebooks', 'Components', 'Tablets', 'Software', 'Phones & PDAs', 'Cameras',
                     'MP3 Players']
    assert (expected_list == categories
...
def test_featured(browser):
    ...
    assert (expected_number_of_elements == len(featured_elements)
...
def test_default_currency(browser):
    ...
    assert (expected_default_currency == current_currency
...
def test_add_random_item_to_basket(browser, item):
    ...
    expected_count = "x 1"
    assert (expected_count in product_item
            ), f"Expected that list contain {expected_count}, actual but the list contains {list}"
    assert any(item in item_product for item_product in product_item
