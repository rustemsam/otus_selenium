...
class CatalogPage(BasePage):
    ...
    def get_list_of_desktops(self) -> list:
        try:
            desktops_search_count = self.wait_helper.wait_for_element(
                self.browser, self.DESKTOP_LIST_CONTAINER
            )
    ...
    def get_count_of_subcategory(self, subcategory: str) -> str:
        try:
            return self.wait_helper.wait_for_element(
                self.browser, self.ACTIVE_SUBCATEGORY.format(subcategory)
            ).text
    ...
    def click_category(self, category: str) -> None:
        try:
            self.wait_helper.wait_for_element(
                self.browser, self.CATEGORY.format(category)
            ).click()
        except NoSuchElementException as e:
    ...
    def get_total_elements_on_page(self):
        total_elements_on_page = self.element_helper.get_text_for_locator(
            self.browser, self.TOTAL_ELEMENTS_ON_PAGE
        )
        match = re.search(r"of\s+(\d+)", total_elements_on_page)
        return int(match.group(1))

    def get_total_elements_for_category(self) -> int:
        total_elements_for_categories = self.element_helper.get_text_for_locator(
            self.browser, self.TOTAL_ELEMENTS_FOR_CATEGORY
        )
    ...
    def sort_by_price_high_to_low(self) -> None:
        try:
            ...
            self.wait_helper.wait_for_element(
                self.browser, self.PRICE_OPTION_HIGH_LOW
            ).click()
        except NoSuchElementException as e:
    ...
    def click_on_product_item(self, item: str) -> None:
        element = self.wait_helper.wait_for_element_to_be_clickable(
            self.browser, self.ITEM_IN_CATALOG.format(item)
        )
    ...
    def get_successfully_text_alert(self) -> str:
        return self.wait_helper.wait_for_element_to_be_clickable(
            self.browser, self.SUCCESSFULLY_ALERT
        ).text

    def add_item_to_card(self) -> None:
        try:
            self.wait_helper.wait_for_element(
                self.browser, self.ADD_ITEM_TO_BASKET
            ).click()
        except NoSuchElementException as e:
...
def add_item_to_wishlist(self) -> None:
    try:
        self.wait_helper.wait_for_element(
            self.browser, self.ADD_ITEM_TO_WISHLIST
        ).click()
    except NoSuchElementException as e:
        print(f"Error when trying to click add item: {e}")
...
def add_item_to_product_comparison(self) -> None:
    try:
        self.wait_helper.wait_for_element(
            self.browser, self.ADD_ITEM_TO_PRODUCT_COMPARISON
        ).click()
    except NoSuchElementException as e:
        print(f"Error when trying to click add item: {e}")
